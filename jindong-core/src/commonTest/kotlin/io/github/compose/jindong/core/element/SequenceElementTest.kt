...
class SequenceElementTest :
  FunSpec({
    ...
    test("collectEvents should handle multiple delays") {
      ...
      element.children.add(DelayElement(durationMs = 50))
      element.children.add(VibrationElement(durationMs = 30, intensity = HapticIntensity.HIGH))
    ...
    test("collectEvents should handle only DelayElements") {
      val element = SequenceElement()
      element.children.add(DelayElement(durationMs = 100))
      element.children.add(DelayElement(durationMs = 200))
    ...
    test("collectEvents should handle complex sequence from documentation example") {
      ...
      // }
      val element = SequenceElement()
      element.children.add(VibrationElement(durationMs = 100, intensity = HapticIntensity.HIGH))
      element.children.add(DelayElement(durationMs = 50))
      element.children.add(VibrationElement(durationMs = 200, intensity = HapticIntensity.HIGH))
    ...
    test("collectEvents should handle nested SequenceElement") {
      ...
      val outerSequence = SequenceElement()
    ...
    test("collectEvents with zero duration delay should not affect timing") {
      val element = SequenceElement()
      element.children.add(VibrationElement(durationMs = 100, intensity = HapticIntensity.STRONG))
      element.children.add(DelayElement(durationMs = 0))
      element.children.add(VibrationElement(durationMs = 100, intensity = HapticIntensity.STRONG))
    ...
    test("totalDurationMs should handle nested SequenceElement") {
      val innerSequence = SequenceElement()
      innerSequence.children.add(VibrationElement(durationMs = 50, intensity = HapticIntensity.MEDIUM))
      innerSequence.children.add(DelayElement(durationMs = 25))

      val outerSequence = SequenceElement()
      outerSequence.children.add(VibrationElement(durationMs = 100, intensity = HapticIntensity.HIGH))
      outerSequence.children.add(innerSequence)
